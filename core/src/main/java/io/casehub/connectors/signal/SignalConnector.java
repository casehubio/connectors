...
public class SignalConnector implements Connector {
    ...
    @ConfigProperty(name = "casehub.signal.api-url")
    Optional<String> apiUrl;

    @ConfigProperty(name = "casehub.signal.number")
    ...
    public boolean send(final ConnectorMessage message) {
        final String url = apiUrl.orElse("");
        final String num = number.orElse("");
        if (url.isBlank() || num.isBlank()) {
        ...
        json.append("\"number\":").append(HttpHelper.jsonQuote(num));
        ...
        return HttpHelper.postJson(url + "/v2/send", json.toString());
    }
}
